...
namespace ClearCore.Cleaner
{
    public class CleanerManager
    {
        ...
        public async Task<long> Clear()
        {
            ...
            Task cl = Task.Factory.StartNew(() =>
            {
                ...
                foreach (Task t in tasks)
                {
                    ...
                    t.Wait();
                    temp++;
                    form.xuiCircleProgressBar1.Invoke(new Action(() => { form.xuiCircleProgressBar1.Value = GetPercent(tasks.Count, temp); }));
                }
            });
            await cl;
            return cleared;
        ...
        public void UpdateUI()
        {
            ...
            form.Invoke(new Action(() => { form.button2.Location = new Point(form.button2.Location.X, 35 + offset); }));
            form.Invoke(new Action(() => { form.button3.Location = new Point(form.button3.Location.X, 35 + offset); }));
            form.Invoke(new Action(() => { form.button4.Location = new Point(form.button4.Location.X, 35 + offset); }));
            form.Invoke(new Action(() => { form.xuiCircleProgressBar1.Location = new Point(form.button1.Location.X, 70 + offset); }));
            form.Invoke(new Action(() => { form.button1.Visible = true; }));
            form.Invoke(new Action(() => { form.button2.Visible = true; }));
            form.Invoke(new Action(() => { form.button3.Visible = true; }));
            form.Invoke(new Action(() => { form.button4.Visible = true; }));
        }
        ...
        public static readonly string DataBaseURL = "https://ztreh.ru/Hds/Cleaner/CleanerUpdater.cs";
